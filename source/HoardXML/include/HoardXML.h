		...
		while((pos=toParse.find("<"))!=toParse.npos) {
			...
			if((newTagPos=tagstring.find_first_not_of("<> \n\t"))!=tagstring.npos) {
				...
				if(tagstring[newTagPos]=='/' ) {
					...
					int endTagNamePos = tagstring.find_first_not_of("<> \n\t/", newTagPos); 
					int endTagNameEnd = tagstring.find_first_of("<> \n\t", endTagNamePos);
					...
					int tagNamePos = tagstring.find_first_not_of("<> \n\t/", newTagPos); 
					int tagNameEnd = tagstring.find_first_of("<> \n\t", tagNamePos);
					...
					int equalPos = attributeString.npos;
					while((equalPos=attributeString.find("="))!=attributeString.npos) {
						...
						int rBegin = attributeString.find_first_of("\"'",equalPos+1);
						int rEnd = attributeString.find_first_of("\"'",rBegin+1);
